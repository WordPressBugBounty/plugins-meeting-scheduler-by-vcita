...
function wpshd_vcita_admin_actions()
{
  if (function_exists('add_menu_page')) {
    add_menu_page(
      esc_html__('vCita Online Scheduling', 'meeting-scheduler-by-vcita'),
      esc_html__('vCita Online Scheduling', 'meeting-scheduler-by-vcita'),
      'edit_posts',
      WPSHD_VCITA_WIDGET_UNIQUE_ID . '/vcita-settings-functions.php',
...
function wpshd_vcita_add_plugins_actions($actions, $plugin_file, $plugin_data, $context)
{
  ...
  if (strpos($plugin_file, WPSHD_VCITA_WIDGET_UNIQUE_ID) !== false) {
    array_unshift($actions, '<a href="' . get_admin_url('','','admin').'admin.php?page='.WPSHD_VCITA_WIDGET_UNIQUE_ID.'/vcita-settings-functions.php' . '" target="_blank">' . esc_html__('Settings', 'meeting-scheduler-by-vcita') . '</a>');
    ...
    if ($wpshd_vcita_widget['uid'] && isset($_COOKIE['wpshd_is_trial']) && $_COOKIE['wpshd_is_trial'] == '1') {
      array_unshift($actions, '<a style="color:red;font-weight:800" href="' . get_admin_url('','','admin').'admin.php?page='.WPSHD_VCITA_WIDGET_UNIQUE_ID.'/vcita-settings-functions.php&vcita-premium' . '" target="_blank">' . esc_html__('Upgrade to pro', 'meeting-scheduler-by-vcita') . '</a>');
    }
  }
